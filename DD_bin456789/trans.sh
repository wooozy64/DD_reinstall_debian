...
install_alpine() {
    ...
    chroot /os setup-timezone -i UTC
    # 3.21 默认是 chrony
...
basic_init() {
    ...
    if is_have_cmd_on_disk $os_dir systemd-firstboot; then
        if chroot $os_dir systemd-firstboot --help | grep -wq '\--force'; then
            chroot $os_dir systemd-firstboot --timezone=UTC --force
        else
            chroot $os_dir systemd-firstboot --timezone=UTC
        fi
    fi
...
EOF
        ...
        purge_legacy_packages_debian $os_dir
...
disable_sftp() {
    ...
    # Keep SSH login, but block SFTP subsystem.
    change_ssh_conf "$os_dir" Subsystem "sftp /bin/false" 02-disable-sftp.conf

    # Remove common SFTP binaries from target system before reboot.
    rm -f "$os_dir/usr/lib/openssh/sftp-server"
    rm -f "$os_dir/usr/libexec/openssh/sftp-server"
    rm -f "$os_dir/usr/bin/sftp"
}
...
purge_legacy_packages_debian() {
    local os_dir=$1
    info 'purge legacy packages'

    chroot_apt_remove "$os_dir" \
        'exim4*' \
        avahi-daemon \
        'cups*' \
        'bluez*' \
