...
if [ -z "$BASH" ]; then
    # alpine 3.20-3.23 没有 bash，先安装 bash 后再用 bash 运行脚本
...
# tee >(grep -iv password >>/reinstall.log)
# 将 stdout 和 stderr 都重定向到 tee
# >(command)这是 Bash 的进程替换（process substitution）。
# 它创建一个匿名管道，并将该管道的文件描述符传递给 command 作为输入或输出。
# 这样，command 可以像处理普通文件一样处理该管道的数据。
# 这种方式避免了使用临时文件，提高了效率。
# echo hi > >(cat) 等价于 echo hi | cat
# exec > something 将当前 shell 的 stdout 重定向到 something
# 朴素的写法：exec > >(tee -a /reinstall.log) 2>&1 但这 不会过滤密码。
exec > >(tee >(grep -iv password >>/reinstall.log)) 2>&1
THIS_SCRIPT=$(readlink -f "$0")
SCRIPT_DIR=$(dirname "$THIS_SCRIPT")
# Use local files in the same directory as this script.
confhome="file://$SCRIPT_DIR"
...
confhome_copy() {
    local name=$1
    local dst=$2
    if is_confhome_local; then
        cp -f "$(confhome_local_path "$name")" "$dst"
    else
...
confhome_cat() {
    local name=$1
    if is_confhome_local; then
        cat "$(confhome_local_path "$name")"
    else
        curl -L "$confhome/$name"
    fi
}
# 这是一个自动定位错误行号 + 错误原因的机制。
# 当脚本执行过程中出现错误时，trap 会捕获 ERR 信号，并调用 trap_err 函数。
# trap_err 函数接收两个参数：错误发生的行号和返回码。
# 它会打印出错误信息，包括行号和返回码，并使用 sed 命令显示出错误发生的具体代码行，帮助调试和定位问题。 
# 这样可以更方便地找出脚本中的错误位置，提升调试效率。
# 注意：set -eE 必须在 trap 之前，否则无法捕获 ERR 信号
# set -Eeuo pipefail
# THIS_SCRIPT="$0"
...
trap_err() {
    ...
    # 通过 sed 命令显示出错误发生的具体代码行，帮助调试和定位问题
...
# 把信息以绿色输出到 stderr，默认会把内容转成 大写并加上 ***** 包裹，但如果第一个参数是 false，就按原样输出。
info() {
    local msg
    if [ "$1" = false ]; then
        shift
...
# 黄色警告信息输出到 stderr，默认会加上 Warning: 前缀，但如果第一个参数是 false，就按原样输出。
warn() {
    local msg
    if [ "$1" = false ]; then
        shift
...
# 这段代码解决了 原生 curl 在自动化脚本中的 6 个现实问题：
# 1. 系统可能没装 curl
# 2. 需要跳过 SSL 证书验证  
# 3. 连接超时
# 4. 404 / 403 默认不算失败
# 5. CentOS 7 curl 版本太旧
# 6. 网络瞬断 / 连接拒绝不会自动重试
# 7. CI / 安装脚本需要稳定退出码
# 8. 调用方不想每次写一堆参数
curl() {
...
# 把点分十进制的子网掩码（netmask）转换成 CIDR 前缀长度。例如：255.255.255.0 → 24
...
# 判断是否在 Windows 上运行，但是这个脚本是给 Linux 系统使用的，因此在 Windows 上运行可能会有很多问题，例如路径分隔符不同、缺少必要的工具和命令、权限问题等等。
is_in_windows() {
...
# 判断 /boot 是否在单独的分区
# 返回值：0 是单独分区，1 不是单独分区
is_boot_in_separate_partition() {
    mount | grep -q ' on /boot type '
}

# 判断系统根目录是否在 btrfs 文件系统中
# 返回值：0 是在 btrfs 中，1 不是在 btrfs 中
is_os_in_btrfs() {
    mount | grep -q ' on / type btrfs '
}

# 判断系统根目录是否在 btrfs 子卷中
# 返回值：0 是在子卷中，1 不是在子卷中
is_os_in_subvol() {
...
# 复制文件到 btrfs 根子卷
# cp_to_btrfs_root /path/to/src1 /path/to/src2 ...
cp_to_btrfs_root() {
...
# 判断某个主机名是否同时有 IPv4 和 IPv6 地址
# is_host_has_ipv4_and_ipv6 example.com
# 返回值：0 是同时有，1 不是
...
# 如果在容器中运行，可能会导致一些命令无法正常工作，例如 fdisk、parted、lsblk 等，这些命令需要访问底层的硬件设备来获取磁盘信息，而在容器中这些设备可能被隔离或虚拟化了，导致命令无法正确获取信息。
# 另外，在容器中运行安装脚本可能会导致一些意外的后果，例如修改了容器的文件系统、网络配置等，而这些修改可能会影响到容器的正常运行，甚至导致容器崩溃。
# 因此，为了避免这些问题，安装脚本通常会断言不在容器中运行，如果检测到在容器中运行，就会提示用户使用专门针对容器设计的工具，例如 OsMutation 来进行系统变更。
# 当然，如果用户非常清楚自己在做什么，并且知道如何在容器中正确地运行安装脚本，那么这个断言可能会显得有些过于严格了，但对于大多数用户来说，这个断言是一个重要的安全措施，可以防止他们在不适合的环境中运行安装脚本，从而避免潜在的问题。
# 当然，如果用户确实需要在容器中运行安装脚本，那么他们可以选择注释掉这个断言函数，但这需要他们对容器环境有足够的了解，并且知道如何处理可能出现的问题。
# 总之，这个断言函数的目的是为了保护用户，防止他们在不适合的环境中运行安装脚本，从而避免潜在的问题和风险。
# 查询是否在容器中运行的命令，例如 systemd-detect-virt -qc、grep container=lxc /proc/1/environ 等，这些命令会检查系统的虚拟化环境，如果检测到是在容器中运行，就会返回相应的结果，从而触发安装脚本中的断言机制，提示用户使用专门针对容器设计的工具来进行系统变更。
# systemd-detect-virt 指令执行输出如下：
# none：表示没有检测到任何虚拟化环境，通常意味着直接在物理机上运行。
# qemu：表示检测到正在 QEMU 虚拟机中运行。
# kvm：表示检测到正在 KVM 虚拟机中运行。`lscpu | grep Hypervisor`、`lsblk`、`lsmod | grep virtio`、`dmesg | grep -i kvm` 也可以检测是否在 kvm 中运行。
# lxc：表示检测到正在 LXC 容器中运行。  
# openvz：表示检测到正在 OpenVZ 容器中运行。 还可以检查 /proc/vz 目录是否存在来判断是否在 OpenVZ 容器中运行。
# docker：表示检测到正在 Docker 容器中运行。
}
...
wmic() {
    ...
    if ! [ -f "$tmp/wmic.ps1" ]; then
        confhome_copy wmic.ps1 "$tmp/wmic.ps1"
    fi
...
# 将语言代码转换为 English Installer 识别的语言名称
english() {
    case "$lang" in
    ar-sa) echo Arabic ;;
...
"
        ...
        if is_use_cloud_image; then
            ...
            if is_confhome_local; then
                eval ${step}_ks=file:///configs/debian.cfg
            else
    ...
    setos_ubuntu() {
        ...
        if is_use_cloud_image; then
            ...
            # ks
            if is_confhome_local; then
                eval ${step}_ks=file:///configs/ubuntu.yaml
            else
...
继续 DD?'
    ...
    setos_centos_almalinux_rocky_fedora() {
        ...
        if is_use_cloud_image; then
            ...
            if is_confhome_local; then
                eval ${step}_ks=file:///configs/redhat.cfg
            else
...
# 验证系统参数,比如 img、image-name、ssh-keys 等
verify_os_args() {
    case "$distro" in
...
# 判断 cloudcone 的 grub 目录是否被链接到另一个 grub 目录
# 返回值：0 是被链接，1 不是被链接
is_grub_dir_linked() {
...
# 只有 linux bios 且 mbr 使用 grub 时，才用本机的 grub
is_use_local_grub() {
    is_use_local_grub_extlinux && is_mbr_using_grub
}

# 只有 linux bios 且 mbr 不使用 grub 时，才用本机的 extlinux
is_use_local_extlinux() {
    is_use_local_grub_extlinux && ! is_mbr_using_grub
}

# 判断 mbr 是否使用 grub 引导
# 返回值：0 是使用 grub 引导，1 不是使用 grub 引导
is_mbr_using_grub() {
    find_main_disk
...
# 查找 grub/extlinux 配置文件
# 参数1：目录
# 参数2：文件名
# 参数3：关键字
# 返回值：找到且只有一个则输出文件路径，否则报错退出
...
echo_tmp_ttys() {
    if false; then
        confhome_cat ttys.sh | sh -s "console="
    else
        case "$basearch" in
...
EOF
    ...
    # 下载 fix-eth-name 脚本
    confhome_copy fix-eth-name.sh fix-eth-name.sh
    confhome_copy fix-eth-name.service fix-eth-name.service

    # 有段时间 kali initrd 删除了原版 wget
    # 但 initrd 的 busybox wget 又不支持 https
    # 因此改成在这里下载
    confhome_copy get-xda.sh get-xda.sh
    confhome_copy ttys.sh ttys.sh
    if [ -n "$frpc_config" ]; then
        confhome_copy get-frpc-url.sh get-frpc-url.sh
        confhome_copy frpc.service frpc.service
    fi
...
# 判断是否能使用云内核
# 参数1：主磁盘设备名，例如 sda nvme0n1 vda
# 参数2..n：网卡设备名，例如 ens3 eth0
# 返回值：0 能使用云内核，1 不能使用云内核
...
mod_initrd() {
    ...
    confhome_copy trans.sh "$initrd_dir/trans.sh"
...
脚本有更新，请重新下载 reinstall.sh"
    ...
    confhome_copy initrd-network.sh "$initrd_dir/initrd-network.sh"
    ...
    if is_confhome_local; then
        for f in debian.cfg ubuntu.yaml redhat.cfg; do
            if [ -f "$SCRIPT_DIR/$f" ]; then
                cp -f "$SCRIPT_DIR/$f" "$initrd_dir/configs/$f"
            fi
        done
    fi
    if [ -n "$frpc_config" ]; then
    ...
    if [ "$distro" = windows ] && [ -n "$custom_infs" ]; then
        ...
        . <(confhome_cat windows-driver-utils.sh)
...
# 脚本入口
###########################
# 判断是否在 Live OS 中运行，Live OS 没有安装程序，因此不能继续执行
...
# 处理命令行参数, 生成 long_opts 字符串
# debug,force-cn,help,passwd:,password:,ssh-port:,ssh-key:,public-key:,web-port:,http-port:,frpc-conf:,frpc-config:,frpc-toml:
long_opts=
...
# 整理参数
# $0 是脚本名，不作为参数传递,$@ 是所有参数
# -n $0：错误信息里用脚本名显示。
# -o "h,x"：短选项，允许 -h、-x。
# --long "$long_opts"：长选项列表（由变量 long_opts 指定，比如 ssh-port:,debug,password:）。
# -- "$@"：把你真正传入的参数交给 getopt 解析，并用 -- 分隔防止歧义。
# 假设传入参数为：./abc.sh --ssh-port 1234 debian 12 --debug --password akdjsad
# 经过 getopt 处理后，opts 的值为： --ssh-port '1234' --debug --password 'akdjsad' -- 'debian' '12'
...
EOF
        ...
        # 只接受特点格式的 ssh key，避免用户误输入了其他字符串
...
if is_in_china && [[ "$confhome" = http*://* ]]; then
